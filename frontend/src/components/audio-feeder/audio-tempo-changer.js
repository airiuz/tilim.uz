...
var AudioTempoChanger = function (opts) {
  ...
  var find_rpeaks = function (mags, res) {
    var max = 0;
    for (var i = 0; i < mags.length; i++) if (mags[i] > max) max = mags[i];
  ...
  obj["process"] = function (in_ar) {
    ...
    if (in_ar.length > 1) {
      ...
      for (var c = 0; c < in_ar.length; c++)
        for (var i = 0; i < in_len; i++) mix[i] += mult * in_ar[c][i];
    }
...
export default AudioTempoChanger;
